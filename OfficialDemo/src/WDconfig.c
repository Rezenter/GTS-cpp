...
/*! \brief	'common_deny' is used to avoid [COMMON] section after [BOARD ...] sections */
int common_deny = 0;
...
static int parseConnections(const char* name, const char* value, WaveDemoConfig_t* WDcfg) {
	...
	if (sscanf(name, "OPEN %u", &bd) == 1) {
		if ((int) bd >= WDcfg->NumBoards) {
			printf("OPEN %u: board index skipped, try to use OPEN %d!\n", bd, WDcfg->NumBoards - 1);
			return 0;
		}
...
int ParseConfigFile(FILE *f_ini, WaveDemoConfig_t *WDcfg) {
	...
	// NumBoards is 1 as default
	// therefore it must be decreased in order to have the correct number of boards read from the file
	if (WDcfg->NumBoards > 1) WDcfg->NumBoards -= 1;
